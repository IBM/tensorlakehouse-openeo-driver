...
class LoadCollectionFromHBase(AbstractLoadCollection):
    ...
    def get_geodn_layer_metadata(
        self, collection_id: str, bands: List[str]
    ) -> List[LayerMetadata]:
...
class LoadCollectionFromCOS(AbstractLoadCollection):
    def load_collection(
        ...
        ) = LoadCollectionFromCOS._group_items_by_band(items=item_search, bands=bands)
            ...
            for band in bands:
                if band in available_bands:
                    ...
                    if asset["href"] not in unique_asset_href:
                        ...
                        if data_asset_description in assets.keys():
                            mediatype_crs_res = (asset["type"], epsg, resolution)
                        else:
                            # group by mediatype
                            mediatype_crs_res = (asset["type"], epsg, resolution)
    ...
    def _load_items_from_cos(
        ...
        if media_type == ZIP_ZARR_MEDIA_TYPE:
            ...
            msg = f"Error! media type is not supported: {media_type}"
            logger.warning(msg)
